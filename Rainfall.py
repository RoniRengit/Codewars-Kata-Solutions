...
def mean(town, strng):
    town_data = [data.split(':') for data in strng.split('\n')]
    town_dic = {towns[0]:towns[1].split(',') for towns in town_data}
    if town_dic.setdefault(town, -1)  == -1:
        return -1
    else:
        precip_list = [values.split(' ') for values in town_dic[town]]
        precip_values = [values[1] for values in precip_list]
        mean = sum(map(float, precip_values))/len(precip_values)
        return mean
...
def variance(town, strng):
    ...
    if town_dic.setdefault(town, -1)  == -1:
        ...
        precip_values = [values[1] for values in precip_list]
        mean = sum(map(float, precip_values))/len(precip_values)
        mean_dev= [((mean - values) ** 2) for values in map(float, precip_values)]
        variance = sum(mean_dev)/len(precip_values)
        return variance
    
# Test cases

print(mean("London", data)) # 51.19(9999999999996) 
print(variance("London", data)) # 57.42(833333333374)
